...
def train_model(csv_path: str, model_save_path: str):
    print("=" * 70)
    print("🐝 Smart Bee - Training Email Intent Detector Model")
    print("=" * 70)
    ...
    X_train, X_test, y_train, y_test = train_test_split(
        df['email'], df['intent'], test_size=0.2, random_state=42, stratify=df['intent']
    )
...
if __name__ == "__main__":
    # Go up 5 levels from train_intent.py to reach Desktop/SmartBeee
    base_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "..", "..", ".."))
    csv_file = os.path.join(base_dir, "email_intent.csv")
    model_file = os.path.join(os.path.dirname(__file__), "intent_model.joblib")
